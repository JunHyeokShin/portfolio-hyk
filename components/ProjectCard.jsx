  ...
  return (
    ...
    >
      ...
      <div className="p-4 rounded-b-lg">
        ...
        <p className="text-sm mb-1 text-gray-600 dark:text-gray-400">{project.description}</p>
        <div className="flex flex-wrap mb-1">
          {project.tags.map((tag) => (
            <span key={tag} className="my-1 mr-1 px-2 border-solid border-2 border-indigo-500 rounded-2xl text-xs text-gray-600 dark:text-gray-400">
              {tag}
            </span>
          ))}
        </div>
        <div>
          ...
          {project.demo && (
            <Link href={`${project.demo}`} target="_blank" className="hover:underline hover:underline-offset-auto text-sm text-indigo-600 dark:text-indigo-400">
              View Demo
            </Link>
          )}
        </div>
